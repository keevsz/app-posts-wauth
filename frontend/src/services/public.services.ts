...
export const login = ({ email, password }: Partial<UserCredentials>) => {
  ...
  return {
    call: axios.post<User>(`${baseURL}/api/user/login`, userCredentials, {
...
export const registerUser = ({
  ...
  return {
    call: axios.post<User>(`${baseURL}/api/user/register`, userData, {
      signal: controller.signal,
    }),
    controller,
  }
...
export const verifyTokenAndGetUser = (token: string) => {
  const response = axios.get<any>(`${baseURL}/api/user/verify-token/${token}`)
  return response
}
...
export const sendEmailToVerify = (email: String) => {
  ...
  return {
    call: axios.post(`${baseURL}/api/user/email-verify`, data, {
      signal: controller.signal,
    }),
    controller,
  }
...
export const sendEmailChangePassword = (email: String) => {
  ...
  return {
    call: axios.post(`${baseURL}/api/user/password-reset`, data, {
      signal: controller.signal,
    }),
    controller,
  }
